...
//EM_BEAM_POSITION[ NEG, POS]
	...
	for (int s = 0; s < 2; s++){ //s=side
		for (int c = 0; c < 16; c++){ //c = channel
			...
			if ((RawDataRPD[s][c][4] > RawDataRPD[s][c][6]) && (RawDataRPD[s][c][4] > RawDataRPD[s][c][7])){ //WARNING THIS IS CUTTING FOR BAD EVENTS!!!!!!!!!!!!!!!!!!!!!!!
				...
				if (s == 0) { //Neg columns and rows
					for (int c = 0; c < 16; c++){
						...
						if (c == 3  || c == 0  || c == 1  || c == 2)  { RPD_COL_N[2] += TS345Array[s][c] * InputJeffWeighter[c];}			  //C3 C2 C1 C0					
					...
					for (int c = 0; c < 16; c++){
						...
						if (c == 14 || c == 13 || c == 12 || c == 15) { RPD_COL_P[1] += TS345Array[s][c] * InputJeffWeighter[c];}			//R3							

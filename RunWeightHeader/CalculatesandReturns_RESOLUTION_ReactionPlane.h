...
double Outputs_Resolution_ReactionPlane_to_Mean( double EventPlane_POS, double EventPlane_NEG){

	double ReactionPlaneResolution_All_mustbeMeaned;
	if (EventPlane_POS != -10 && EventPlane_NEG != -10){

		ReactionPlaneResolution_All_mustbeMeaned = sqrt(cos(abs(EventPlane_POS - EventPlane_NEG))); //IDK if it matters wether neg or pos is first
		...
		return ReactionPlaneResolution_All_mustbeMeaned = -10;
	}
}

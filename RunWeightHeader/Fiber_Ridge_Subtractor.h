...
void Fiber_Ridge_Subtractor_Outputs_Array( double RawDataRPD[2][16][10], double &TESTVALUE,double Percentage_to_Subtract, double (&Ridge_Subtracted_RPD_Data)[2][16]){
	...
	for (int r = 0; r < 4; r++){
		for (int s = 0; s < 2; s++){
			for (int c = 0; c < 4; c++){
				if (r != 0){
				Percentage_Subtracted_Storage[s][c] = TS345Array[s][LookUpTable[(r-1)][s][c]] * Percentage_to_Subtract;
				}

				Ridge_Subtracted_RPD_Data[s][(c+4*r)] = TS345Array[s][LookUpTable[r][s][c]] - Percentage_Subtracted_Storage[s][c];

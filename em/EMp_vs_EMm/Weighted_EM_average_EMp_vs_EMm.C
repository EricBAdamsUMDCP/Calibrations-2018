...
void Weighted_EM_average_EMp_vs_EMm(int runnumber=326776){
  initRootStyle();
  cout << "Running SOFTWARE: Weighted_EM_average_EMp_vs_EMm.C 3/15/19 3:49" << endl;
  ...
  TH2F* EMp_vs_EMm;
  EMp_vs_EMm = new TH2F(Form("EMp_vs_EMm_%d", runnumber), Form("EMp_vs_EMm_%d", runnumber), 45, 1, 5, 45, 1 ,5); //creating new 2Dhist //75
...
//commenting out main funct8ions for other histos because they take time uncomment its yo boi comment to make them appear
  ...
  /*its yo boi comment for(int iside = 0; iside < 2; iside++)
    for(int itype = 0; itype < 2; itype++){

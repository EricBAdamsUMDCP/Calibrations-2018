...
void Weighted_EM_average_EMp_vs_EMm_IN_cm(int runnumber=326776){
  initRootStyle();
  cout << "Running SOFTWARE: Weighted_EM_average_EMp_vs_EMm_IN_cm.C 4/9/19 5:07" << endl;
  ...
  TH2F* EMp_vs_EMm;
  EMp_vs_EMm = new TH2F(Form("EMp_vs_EMm_%d", runnumber), Form("EMp_vs_EMm_%d", runnumber), 45, -5, 5, 45, -5 ,5); //creating new 2Dhist //75
  
  
  for(int iside = 0; iside < 2; iside++)
    for(int itype = 0; itype < 2; itype++){
        Weighted_average_EM[iside][itype] = new TH1F(Form("Weighted_average_%s%s",ttit[itype],stit2[iside]),Form("%s%s_%d; Channel ;Entries",ttit[itype],stit[iside],runnumber),500,-5, 5); // was 0 and 6 before
      }
    ...
    for(int n = 0; n < 50; n++){
      ...
      if(type == 0){ // EM section
        if(side == 1){
			...
			double sumWeightEMPos =  (EMChannel0P*EM[0]) + (EMChannel1P*EM[1]) + (EMChannel2P*EM[2]) + (EMChannel3P*EM[3]) + (EMChannel4P*EM[4]);
			double sumWeightEMNeg =  (EMChannel0N*EM[0]) + (EMChannel1N*EM[1]) + (EMChannel2N*EM[2]) + (EMChannel3N*EM[3]) + (EMChannel4N*EM[4]);
			Weighted_average_EM[1][0]-> Fill(sumWeightEMPos/sumEMPos);
			Weighted_average_EM[0][0]-> Fill(sumWeightEMNeg/sumEMNeg);
			
			EMp_vs_EMm-> Fill(sumWeightEMPos/sumEMPos, sumWeightEMNeg/sumEMNeg); //filling new 2d
  }
...
//commenting out main funct8ions for other histos because they take time uncomment its yo boi comment to make them appear
 ...
 for(int iside = 0; iside < 2; iside++)
    ...
    //    gauss->Draw("l same");
	  ...
	  EMp_vs_EMm->Draw("colz");
	  //TLine *line = new TLine(3.533, 0, 3.533, 5);
	  //line->Draw();
	  c1->SaveAs(Form("ZDC_figures/EMp_vs_EMm/EMp_vs_EMm_%d.png", runnumber));
	 // line->Draw();

...
void Fixed_Uncut_v_Cut_HADcut_RPDcut_ALLpulse_QBLoop(int runnumber=326776){
  initRootStyle();
  ...
  const int NRPD=16; 
  float RPDCorrect[NSIDE][NRPD] = {
								   {11.0, 15.0, 2.0, 5.0, 8.0, 12.0, 1.0, 6.0, 9.0, 13.0, 0.0, 4.0, 10.0, 14.0, 3.0, 7.0},
			...
			for(int i =0; i < 2; i++){ //problem for loop refuses to stop
				//cout << "line after pos neg loop" << endl;
					for(int j=0 ; j < 16; j++){
								...
								c4->cd(RPDCorrectBlockOrderFORPLOTTING[i][j]);
								cout << "line after cd="<< (RPDCorrectBlockOrderFORPLOTTING[i][j]) << endl; // this is working
								cout << "j" << j << endl; 
								cout << "i" << i << endl;
								gPad->SetLogy();

...
void Uncut_v_Cut_HADcut_RPDcut_QBC(int runnumber=327560){
  initRootStyle();
   ...
   TH1F* fC_RPD[2][16];
	...
	for(int i = 0; i < 2; i++){
		for(int j = 0; j < 2; j++)
		  for(int k = 0; k < 5; k++){
			...
			fC_ZDC[i][j][k] = new TH1F(Form("327560 fC ZDC %s%s ch %d",ttit[j],stit[i],k+1),Form("ZDC %s%s channel %d;TS [25 ns];Q [fC]",ttit[j],stit[i],k+1),10,-0.5,9.5); 
		  }

		for(int j = 0; j < 16; j++){
		  
		  fC_RPD[i][j] = new TH1F(Form("327560 fC RPD%s ch %d",stit[i],j+1),Form("RPD%s channel %d;TS [25 ns];Q [fC]",stit[i],j+1),10,-0.5,9.5);
		}
	  ...
	  TH1F* fC_RPDHT[2][16];
		...
		for(int j = 0; j < 2; j++)
		  for(int k = 0; k < 5; k++){
			...
			fC_ZDCHT[i][j][k] = new TH1F(Form("327560 fC ZDCHT %s%s ch %d",ttit[j],stit[i],k+1),Form("ZDCHT %s%s channel %d;TS [25 ns];Q [fC]",ttit[j],stit[i],k+1),10,-0.5,9.5); 
		  }

		for(int j = 0; j < 16; j++){
		  
		  fC_RPDHT[i][j] = new TH1F(Form("327560 fC RPDHT%s ch %d",stit[i],j+1),Form("RPDHT%s channel %d;TS [25 ns];Q [fC]",stit[i],j+1),10,-0.5,9.5);
		}
	...
	for(int i = 0; i < 2; i++){
		for(int j = 0; j < 2; j++)
		  ...
		  fC_ZDC_Pure[i][j][k] = new TH1F(Form("327560 fC ZDC%s%s ch %d Pure",ttit[j],stit[i],k+1),Form("ZDC Pure %s%s channel %d;TS [25 ns];Q [fC]",ttit[j],stit[i],k+1),10,-0.5,9.5); 
		  }

		for(int j = 0; j < 16; j++){
		  
		  fC_RPD_Pure[i][j] = new TH1F(Form("327560 fC RPD%s ch %d Pure",stit[i],j+1),Form("RPD%s Pure channel %d;TS [25 ns];Q [fC]",stit[i],j+1),10,-0.5,9.5);
		}
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  TFile f2("ratio.root","RECREATE"); 
	...
	*/
	...
	/////////////////////////////////
		/*HAD*/ HAD112_CH1_Neg->SetLineColor(4); // blue
				...
				HAD112_CH1_Pos->SetLineColor(2/*3*/);//Red//green
				HAD112_CH1_Pos->SetFillColorAlpha(kRed/*kGreen*/, 0.35);
...
////////////////////////////////////////////////////////////////////////////////////////////////
   TCanvas *c4 = new TCanvas("c4", "RUN 327560", 2000, 1500);
		...
		c4->Divide(4,4,0.0001,0.0001); //assumes canvas is called c1 and creates a different canvas c2 then divides canvas into p by p matrix
			...
			fC_RPD_Pure[1][11]->SetLineColor(4);
			fC_RPD[1][11]->SetLineColor(B);
			fC_RPD_Pure[1][11]->Draw("hist e");
			fC_RPD[1][11]->Draw("same");
			c4->cd(2);
			gPad->SetLogy();
			//fC_RPD[1][15]->Draw("hist e");
			fC_RPD_Pure[1][15]->SetLineColor(4);
			fC_RPD[1][15]->SetLineColor(B);
			fC_RPD_Pure[1][15]->Draw("hist e");
			fC_RPD[1][15]->Draw("same");
			c4->cd(3);
			gPad->SetLogy();
			...
			fC_RPD_Pure[0][15]->SetLineColor(4);
			fC_RPD[0][15]->SetLineColor(B);
			fC_RPD_Pure[0][15]->Draw("hist e");
			fC_RPD[0][15]->Draw("same");
			c4->cd(3);
			gPad->SetLogy();
...
/*
			...
			//	 c4->Update(); 
			  // }
			   c4->SaveAs(Form("ZDC_figures/Uncut_v_Cut_HADcut_RPDcut_QBC_327560/HAD_BadSignalSearch_CORRECTQBLAYOUT_fC_RPDHT_Plus_channel_ALL_327560.png"));
			...
			fC_RPDHT[0][7]->Draw("hist e");
			  // for(int i = 1; i < 17; i++){
				...
				// c4->Update(); 
			  // }
			   c4->SaveAs(Form("ZDC_figures/Uncut_v_Cut_HADcut_RPDcut_QBC_327560/HAD_BadSignalSearch_CORRECTQBLAYOUT_fC_RPDHT_Minus_channel_ALL_327560.png"));
*/
...
/*
	TCanvas *c5 = new TCanvas("c5", "RUN 327560", 2000, 2000);
		c5->Divide(5,4,0.0001,0.0001); //x=5 y =2 i think this allows for row 1 = em r2 = had 
			...
			c5->Update();  
			c5->SaveAs(Form("ZDC_figures/Uncut_v_Cut_HADcut_RPDcut_QBC_327560/HAD_BadSignalSearch_fC_ZDC_Negative.png"));
			...
			c5->Update();  
			c5->SaveAs(Form("ZDC_figures/Uncut_v_Cut_HADcut_RPDcut_QBC_327560/HAD_BadSignalSearch_fC_ZDC_Positive.png"));
			...
			c5->Update();  
			c5->SaveAs(Form("ZDC_figures/Uncut_v_Cut_HADcut_RPDcut_QBC_327560/HAD_BadSignalSearch_fC_ZDCHT_Negative.png"));
																					//c5->SaveAs(Form("ZDC_figures/RPD_45456_Ratio_Run/fC_ZDC_%s_%s_channel_%d.png",ttit[itype],stit2[iside],ich+1));
			//////////////////	
			//Pos EM AND HAD//
			//////////////////
			...
			c5->Update();  
			c5->SaveAs(Form("ZDC_figures/Uncut_v_Cut_HADcut_RPDcut_QBC_327560/HAD_BadSignalSearch_fC_ZDCHT_Positive.png"));
*/
			f2.Write();

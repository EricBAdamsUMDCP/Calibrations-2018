...
void RPD_TS_RATIOS_45_456_EventSelector_4by4_RPDTSDistGreaterCut(int runnumber=286178){
  initRootStyle();
  //CUTOFF VALUE FOR GENERATING fC VS TS HISTOS SET BELOW//
  double CutOff = .7;////SWITCHED CUT OFF TO BE GRETER THAN!!
...
////////////////////////////////////////////////////////////////////////////////////////////////
   TCanvas *c4 = new TCanvas("c4", "RUN 327560");
	   c4->Divide(4,4); //assumes canvas is called c1 and creates a different canvas c2 then divides canvas into p by p matrix
			   ...
			   for(int i = 1; i < 17; i++){
				c4->cd(i); //(this sets the current canvas division to canvas divsion i) assumes loop variable is i, and inputs them into the canvas from left to right then down on row
				 gPad->SetLogy(); // HOW TO SET LOG OF GRAPHS IN A CANVAS WHEN HSITOS ARE NOT INATELY LOG
				 fC_RPD[0][i-1]->Draw();
				 c4->Update(); 
			   }
			   c4->SaveAs(Form("ZDC_figures/RPD_45456_Ratio_Run/4x4_5x2_RPDZDC_TS/fC_RPD_Minus_channel_ALL_327560.png"));
			   ...
			   c4->SaveAs(Form("ZDC_figures/RPD_45456_Ratio_Run/4x4_5x2_RPDZDC_TS/fC_RPD_Plus_channel_ALL_327560.png"));

	TCanvas *c5 = new TCanvas("c5", "RUN 327560");
		c5->Divide(5,4); //x=5 y =2 i think this allows for row 1 = em r2 = had 

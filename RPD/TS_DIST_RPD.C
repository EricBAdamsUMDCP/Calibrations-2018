...
//main function macro
void TS_DIST_RPD(int runnumber=326776){
	initRootStyle();
	...
	//s stands for side
		...
		//c stands for channel
			fC_RPD[s][c]      = new TH1D(Form("fC RPD%s channel %d %d", stit[s], c+1, runnumber),Form("326776Weighted_RPD%s channel %d %d;TS [25 ns];Q [fC]",stit[s],c+1, runnumber),NumberOfBins,0/* MinXTH1F */,9 /* ArrayMaxXTH1F[s][c] */); /// lower bound is set higher than upper bound bc when rene burn designed this thing HE MADE THAT THE ONLY GOD DAMN WAY FOR IT TO AUTOFIT THE AXES .....
	...
	for(int i =0; i < 2; i++){ 
		for(int j=0 ; j < 16; j++){
			...
			fC_RPD[i][j]->Draw("hist e"); //->Draw("same");
			 ...
			 c4->SaveAs(Form("ZDC_figures/TS_Dist_RPD/WCUT_RPD_channel_%d_side_%s_runnumber_%d.png", j+1, stit2[i], runnumber)); // potential bug area
		} 
	}//

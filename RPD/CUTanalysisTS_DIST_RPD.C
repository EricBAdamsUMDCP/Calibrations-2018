...
//main function macro
void CUTanalysisTS_DIST_RPD(int runnumber=326776){
	initRootStyle();
	
	cout << "Running SOFTWARE: CUTanalysisTS_DIST_RPD.C  Edited: 6/26/2019 2:15:18 PM" << endl;
	...
	const int NumberOfBins = 10; //number of bins in TH1F histograms produced by this code
	...
	//s stands for side
		...
		//c stands for channel
			fC_RPD[s][c] = new TH1D(Form("fC RPD%s channel %d %d", stit[s], c+1, runnumber),Form("326776Weighted_RPD%s channel %d %d;TS [25 ns];Q [fC]",stit[s],c+1, runnumber), 16 /*NumberOfBins*/, 0, 16/*9*/ ); 
			//fC_RPD_Pure[s][c] = new TH1D(Form("fC RPD%s Pure channel %d %d", stit[s], c+1, runnumber),Form("RPD%s Pure channel %d %d;TS [25 ns];Q [fC]",stit[s],c+1, runnumber),NumberOfBins,0,9);
		}
	}
	...
	/// END Declaring TLeaf... /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	double totalnumber = ZDCDigiTree->GetEntries();
	double rejected = 0;
	...
	for ( int i = 0;  i < ZDCDigiTree->GetEntries(); i++ ){
		...
		for ( int s = 0; s < 2; s++){
			for (int c = 0; c < 16; c++){
				...
				//appeares to be optimal cut removes the most crap while sparing the most data
				if ( (RawDataRPD[s][c][4] > RawDataRPD[s][c][6]) && (RawDataRPD[s][c][4] > RawDataRPD[s][c][7])) /*&& (RawDataRPD[s][c][5] > RawDataRPD[s][c][7]) //takes out too much ts 4 */{
	...
	for(int i =0; i < 2; i++){ 
		//for(int j=0 ; j < 16; j++){
			...
			fC_RPD[i][0]->SetLineColor(B);
			fC_RPD[i][0]->Draw("hist e"); //->Draw("same");

			 c4->SaveAs(Form("ZDC_figures/TS_Dist_RPD/all_non_cutfit_channel_quantity_RPD_channel_%d_side_%s_runnumber_%d.png", 0/*j+1*/, stit2[i], runnumber)); // potential bug area
		//} 
	}//

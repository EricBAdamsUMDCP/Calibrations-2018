...
void RPD_TS_RATIOS_45_456_and_HAD1_RATIO_EventSelector_4by4_RPDTSDistCORRECTQBLAYOUT(int runnumber=286178){
  initRootStyle();
  //CUTOFF VALUE FOR GENERATING fC VS TS HISTOS SET BELOW//
  double CutOff = .6;////
  //CUTOFF VALUE FOR GENERATING fC VS TS HISTOS SET ABOVE//
  
  cout << "Running SOFTWARE: RPD_TS_RATIOS_45_456_and_HAD1_RATIO_EventSelector_4by4_RPDTSDistCORRECTQBLAYOUT" << endl;
...
//declaring variables to hold channel charge for 45 and 456 for all RPD pos and neg
	...
	if(0.45 < signal1/signal12 && signal1/signal12 < 0.55){ //range is peak of HAD ratio excluding 0 and 1 ratio peaks as this implies either there is little prepulse of the prepulse is equal in 0 and 1. Our observations have shown the prepulse is generally veryy large in TS 1 and minimal in TS 2
	for(int iTS = 0; iTS < 10; iTS++){
			if(type != 3){ // EM or HAD section
			  double value = fCvTSleaf[iTS]->GetValue(n);
			  if(value < 0){ //!!!! I AM CORRECT THIS IF STATEMENT REMOVES NEGATIVE VALUES!!!
			  value = 0;}
			  //if(fCvTSleaf[iTS] < 0) 
			  fC_ZDCHT[side][type][channel]->Fill(iTS,value);    //uncommenting TS to see what happens      
		//      event->SetBinContent(iTS+1,fCleaf[iTS]->GetValue(n));
			}
			else{ // RPD section
			  double value = fCvTSleaf[iTS]->GetValue(n);
  ...
  TFile f2("ratio.root","RECREATE"); 
    ...
    //pos drawing and saving histos
		/*HAD*/ HAD112_CH1_Pos->SetLineColor(4);
				...
				c1->SaveAs("ZDC_figures/RPD_45456_Ratio_Run/HAD112_CH1_Pos.png");
	...
	/////////////////////////////////
		/*HAD*/ HAD112_CH1_Neg->SetLineColor(4);
					...
					c1->SaveAs("ZDC_figures/RPD_45456_Ratio_Run/HAD112_CH1_Neg.png");
   ...
   TCanvas *c4 = new TCanvas("c4", "RUN 327560", 2000, 1500);
        c4->Divide(4,4,0.0001,0.0001); //assumes canvas is called c1 and creates a different canvas c2 then divides canvas into p by p matrix
			...
			fC_RPDHT[1][7]->Draw("hist e");
			   ...
			   c4->SaveAs(Form("ZDC_figures/RPD_45456_Ratio_Run/4x4_5x2_RPDZDC_TS/CORRECTQBLAYOUT_fC_RPDHT_Plus_channel_ALL_327560.png"));
			...
			fC_RPDHT[0][7]->Draw("hist e");
			  /* for(int i = 1; i < 17; i++){
			   ...
			   c4->SaveAs(Form("ZDC_figures/RPD_45456_Ratio_Run/4x4_5x2_RPDZDC_TS/CORRECTQBLAYOUT_fC_RPDHT_Minus_channel_ALL_327560.png"));
	...
	TCanvas *c5 = new TCanvas("c5", "RUN 327560", 2000, 2000);
		c5->Divide(5,4,0.0001,0.0001); //x=5 y =2 i think this allows for row 1 = em r2 = had 
			...
			c5->Update();  
			c5->SaveAs(Form("ZDC_figures/RPD_45456_Ratio_Run/4x4_5x2_RPDZDC_TS/fC_ZDCHT_Negative.png"));
																					//c5->SaveAs(Form("ZDC_figures/RPD_45456_Ratio_Run/fC_ZDC_%s_%s_channel_%d.png",ttit[itype],stit2[iside],ich+1));
			//////////////////	
			//Pos EM AND HAD//
			//////////////////
			...
			c5->Update();  
			c5->SaveAs(Form("ZDC_figures/RPD_45456_Ratio_Run/4x4_5x2_RPDZDC_TS/fC_ZDCHT_Positive.png"));

...
//main function macro
void COPYPASTERPD_Beam_Position_header_test_playground(int runnumber = 326776 ){
	initRootStyle();
	string Dataset = "AOD_zdc_digi_tree_326776_many_3";
	cout << "Running SOFTWARE: COPYPASTERPD_Beam_Position_header_test_playground.C 4/15/19 5:15" << endl;
	...
	for (int i = 0; i < ZDCDigiTree->GetEntries(); i++) {
		...
		if (n == 49){
		RPD_P_BEAM_X1D->Fill(WeightedAverageRPD_P_X);

		RPD_P_BEAM->Fill( WeightedAverageRPD_P_X, WeightedAverageRPD_P_Y);
		}		
		if (n==33){
		RPD_N_BEAM_X1D->Fill(WeightedAverageRPD_N_X);
					
		RPD_N_BEAM->Fill( WeightedAverageRPD_N_X , WeightedAverageRPD_P_Y);	
		}
		}	

...
//main function macro
void Fixed_RPDWEIGHTGEANT_AND_REALWEIGHTS_counts_v_fC_QBLoop(int runnumber=326776)
{
	initRootStyle();
	
	cout << "Running SOFTWARE: Fixed_RPDWEIGHTSGEANT_&_REALWEIGHTS_count_v_fC_QBLoop.C 4/8/19 1:22" << endl;
	...
	const double MaxXTH1F = 350000.0;
	...
	float DATAWeightsFromRun_326776[NSIDE][NRPD] = { //data weights from run 326776 v3 many
													{41.2312, 15.3597, 6.20754, 34.7239, 0.468297, 4.14262, 0.303433, 0.647079, 0.165138, 0.186473, 0.61339, 0.154575, 2.3042, 1.94258, 0.608618, 0.421212}, //neg
													{279.198, 38.239, 1.35442, 6.65515, 34.589, 6.7979, 3.71134, 0.207054, 0.276156, 0.299668, 0.145468, 0.165878, 1.68038, 1.69571, 0.9093, 13.9165} //pos
												   }; //may be faulty weights recalcualte

	double RPD_Pos_TS456[NRPD] = {0}; //found bug channels in wrong order lower down RPDCorrectBlockOrder not implemented /// fixed??? 4/8/19 1:57
	double RPD_Neg_TS456[NRPD] = {0}; //found bug channels in wrong order lower down RPDCorrectBlockOrder not implemented /// fixed??? 4/8/19 1:57
		...
		for( int n = 0; n < NChannels; n++){ //iterates through all channels of both ZDC + and -
			...
			if ((fC_of_TS456_Summed > 50) && (fC_of_TS45_Summed/fC_of_TS456_Summed > .8) && (TS_Seven <= TS_Five) && (TS_One/TS_Zero < 1000) && (TS_Zero != 0)){
				...
				if (type == 3){
					...
					fC_RPD[side][channel]->Fill(fC_of_TS456_Summed, DATAWeightsFromRun_326776[side][channel]/*Jeffsweights[side][channel]*/);
	...
	for ( int c = 0; c < 16; c++){
		for ( int NBin = 0; NBin < NumberOfBins; NBin++) { 
		RPD_Pos_TS456[c] = fC_RPD_Pure[1][c]->GetBinContent(NBin);
		}
	}
	
	for ( int c = 0; c < 16; c++){
		for ( int NBin = 0; NBin < NumberOfBins; NBin++) { 
		RPD_Neg_TS456[c] = fC_RPD_Pure[0][c]->GetBinContent(NBin);
		}
	}
	...
	//NEG = 0
		...
		cout << "Neg Channel RPDDataWeight " << QuartzBlockOrdering+1 << ":  " << RPDDataWeight << "  Calculated N: " << N << " Jeffsweight" << Jeffsweights[0][c] << endl;
	}
	...
	//Pos = 1
		...
		cout << "Pos Channel RPDDataWeight " << QuartzBlockOrdering+1 << ":  " << RPDDataWeight << "  Calculated N: " << N << " Jeffsweight" << Jeffsweights[1][c] << endl;
	}
	...
	for(int i =0; i < 2; i++){ 
		for(int j=0 ; j < 16; j++){
			...
			c4->cd(RPDCorrectBlockOrder[i][j] + 1);
			gPad->SetLogy();

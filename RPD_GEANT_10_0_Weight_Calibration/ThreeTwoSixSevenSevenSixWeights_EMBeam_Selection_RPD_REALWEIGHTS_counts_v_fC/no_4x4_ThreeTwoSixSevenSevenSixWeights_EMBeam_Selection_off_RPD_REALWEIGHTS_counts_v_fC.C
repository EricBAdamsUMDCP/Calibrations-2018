...
//main function macro
void no_4x4_ThreeTwoSixSevenSevenSixWeights_EMBeam_Selection_off_RPD_REALWEIGHTS_counts_v_fC(int runnumber=326776)
{
	initRootStyle();
	
	cout << "Running SOFTWARE: no_4x4_ThreeTwoSixSevenSevenSixWeights_EMBeam_Selection_off_RPD_REALWEIGHTS_counts_v_fC.C 4/11/19 4:00" << endl;
	...
	for ( int i = 0;  i < ZDCDigiTree->GetEntries(); i++ ){
		...
		for ( int n = 0; n < NChannels; n++ ){ //iterates through all channels of both ZDC + and -
			...
			//experimental pointer code to allow the em beam headerr file to be aware of variablers outside of its scope
			
			/// not implemented added more variables to function... the shitty solution
	...
	for(int i =0; i < 2; i++){ 
		for(int j=0 ; j < 16; j++){
			...
			title->Draw(); */
			 ...
			 c4->SaveAs(Form("ZDC_figures/Data_Weights326776/Solo_RPD_%d_Weighted_10_0_%s_fCvCounts_%d.png", j+1, stit2[i], runnumber)); // potential bug area
		} 
		...
		for(int j=0 ; j < 16; j++){
			...
			hsRPD[i][j]->Add(fC_RPD_Array_Plot[i][j/*-1*/]);
			fC_RPD_Pure_Array_Plot[i][j/*-1*/]->SetLineColor(A);
			/*
			fC_RPD_Pure[i][j-1]->SetFillStyle(3001);
			fC_RPD_Pure[i][j-1]->SetFillColorAlpha(kBlue, 0.35); */
			hsRPD[i][j]->Add(fC_RPD_Pure_Array_Plot[i][j/*-1*/]);
			//cout << "here5" << endl;
			...
			//lt->DrawLatexNDC(0.5,0.5, Form("Channel_%d", j+1) ); // maay cause bug be wary
			//hs->SetTitle(Form("Channel_%d", j + 1));
			//hsRPD[i][j]->GetXaxis()->SetRange(0,1000); /// added to force root to set the axis to what i want
			TPad *newpad=new TPad("newpad","a transparent pad",0,0,1,1);
			newpad->SetFillStyle(4000);
			newpad->Draw();
			newpad->cd();
			 ...
			 */
			 ...
			 c4->SaveAs(Form("ZDC_figures/Data_Weights326776/RPD_%d_Weighted_10_0_%s_fCvCounts_%d.png", j+1, stit2[i], runnumber)); // potential bug area
		} 
